...
func CreateWindowsUniversalAppXFromDiscriminatorValue(
...
func (m *WindowsUniversalAppX) Serialize(
	...
	if m.GetCommittedContainedApps() != nil {
		cast := make(
			...
			len(m.GetCommittedContainedApps()),
		)
	...
	}
	{
		err = writer.WriteStringValue(
			...
			m.GetIdentityResourceIdentifier(),
		)
		if err != nil {
			return err
		}
	...
	}
	{
		err = writer.WriteObjectValue(
			...
			m.GetMinimumSupportedOperatingSystem(),
		)
		if err != nil {
			return err
		}
...
func (m *WindowsUniversalAppX) SetMinimumSupportedOperatingSystem(
	value WindowsMinimumOperatingSystemable,
) {
